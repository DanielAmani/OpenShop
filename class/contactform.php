...
class contactForm{
	...
	function contactForm($cfg)
	{
		...
		// � (keeps utf-8 encoding in the file)
	...
	function sendMail($param)
	{
		...
		/* $_SERVER['SCRIPT_NAME'] not used anymore because the procedure below would only return "My-Contact-Form-9/cfg-contactform-9/" without the page name
		...
		*/
		...
		$mail_body = $this->cfg['adminnotification_subject'].': '.@date("F jS, Y, G:i")
							...
							.($form_url?$this->mail_line_break.$this->mail_line_break.'Form URL: '.$this->mail_line_break.$form_url : '')						
